...
graphToNetwork(Graph) ->
    ...
    RootPid = nodeSpawn(Graph, Node_Table,
			IncomingEdges_Count_Table, []),
...
nodeSpawn([], Node_Table, _IncomingEdges_Count_Table,
	  Node_List) ->
    % get the first node in the Graph
    First_Node_Name = lists:last(Node_List),
    % get the RootPid
    [{_RootName, RootPid}] = ets:lookup(Node_Table,
					First_Node_Name),
    % return RootPid
    RootPid;
nodeSpawn([First | Rest], Node_Table,
	  IncomingEdges_Count_Table, Node_List) ->
    ...
    lists:foreach(fun (Edge) ->
			  ...
			  Result = ets:lookup(IncomingEdges_Count_Table,
					      EdgeToNodeName),
			  if Result == [] ->
				 ets:insert(IncomingEdges_Count_Table,
					    {EdgeToNodeName, 1});
			     true ->
				 ets:update_counter(IncomingEdges_Count_Table,
						    EdgeToNodeName, 1)
			  end
		  end,
		  % Obj = ets:match_object(IncomingEdges_Count_Table,{'$0','$1'}),
    ...
    nodeSpawn(Rest, Node_Table, IncomingEdges_Count_Table,
	      New_Node_List).

initial_network_config([], _Node_Table,
		       _IncomingEdges_Count_Table) ->
    true;
initial_network_config([First | Rest], Node_Table,
		       IncomingEdges_Count_Table) ->
    {RouterNodeName, Edges} = First,
    [{_RouterNodeName, RouterNodePid}] =
	ets:lookup(Node_Table, RouterNodeName),
    [{_RouterNodeName, NoInEdge}] =
	ets:lookup(IncomingEdges_Count_Table, RouterNodeName),
    send_initial_control_message(Edges, RouterNodePid,
				 Node_Table, [], NoInEdge),
    initial_network_config(Rest, Node_Table,
			   IncomingEdges_Count_Table).

send_initial_control_message([], RouterNodePid,
			     _Node_Table, ListOfNames, NoInEdge) ->
...
send_initial_control_message([FirstEdge | RestEdges],
			     RouterNodePid, Node_Table, ListOfNames,
			     NoInEdge) ->
    {DestName, Names} = FirstEdge,
    [{_NodeName, DestPid}] = ets:lookup(Node_Table,
					DestName),
    Names_with_Pid = get_list_of_names(Names, DestPid, []),
    New_ListOfNames = Names_with_Pid ++ ListOfNames,
    send_initial_control_message(RestEdges, RouterNodePid,
				 Node_Table, New_ListOfNames, NoInEdge).

get_list_of_names([], _DestPid, ListOfNames) ->
    ListOfNames;
get_list_of_names([FirstName | RestNames], DestPid,
		  ListOfNames) ->
...
extendNetwork(RootPid, SeqNum, From,
	      {NodeName, Edges}) ->
    ControlPid = self(),
    % get the list of routing entries and list of nodes who need to update its NoInEdge
    {Routing_List, IncomingEdgeNode_List} =
	get_routing_entries_and_edges_list(Edges, [], []),
    % send control message to the root router node
    RootPid !
      {control, self(), ControlPid, SeqNum,
       fun (Name, Table) ->
	       ...
	       if From == Name ->
		      ...
		      NewSpawnPid !
			{control, self(), ControlPid, 0,
			 fun (_SpawnedNodeName, SpawnedNodeTable) ->
				 ets:insert(SpawnedNodeTable,
					    {'$NoInEdges', 1}),
		  ...
		  true ->
	       ...
	       NeedUpdateNoInEdge = lists:member(self(),
...
get_routing_entries_and_edges_list([], ListOfNames,
				   IncomingEdgeNode_List) ->
    {ListOfNames, IncomingEdgeNode_List};
get_routing_entries_and_edges_list([FirstEdge
				    | RestEdges],
				   ListOfNames, IncomingEdgeNode_List) ->
    ...
    Names_with_Pid = get_list_of_names(Names, EdgeToNodePid,
				       []),
    New_ListOfNames = Names_with_Pid ++ ListOfNames,
    New_IncomingEdgeNode_List = IncomingEdgeNode_List ++
				  [EdgeToNodePid],
    get_routing_entries_and_edges_list(RestEdges,
				       New_ListOfNames,
				       New_IncomingEdgeNode_List).
