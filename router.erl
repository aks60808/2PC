...
process(RouterName, Table, Cur_SeqNum, IsIn2PC,
	MsgQueue, TentativeResult) ->
    % proceess the queued msgs
    if IsIn2PC =/= true andalso length(MsgQueue) =/= 0 ->
	   lists:foreach(fun (Msg) -> self() ! Msg end, MsgQueue),
	   process(RouterName, Table, Cur_SeqNum, IsIn2PC, [],
		   TentativeResult);
       true -> ok
    end,
    ...
    receive
	  ...
	  when Dest == RouterName ->
	  % I am the Dest node
	  Message = {message, Dest, From, Pid, Trace},
	  if IsIn2PC == true -> % if I am in 2PC
		 % queue this msg
	  ...
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  New_MsgQueue, TentativeResult);
      {can_you_commit, DestNodeName, FromPid, RootNodeName,
       Trace}
	  when DestNodeName == RouterName ->
	  io:format("~w got canyoucommit from ~w~n",
		    [RouterName, FromPid]),
	  ...
	  if Children == abort ->
		 LastPid !
		   {i_cannot_commit, RootNodeName, self(), RouterName,
		    Rest};
	     true ->
		 LastPid !
		   {i_can_commit, RootNodeName, self(), RouterName, Rest}
	  end,
	  % keep the status ( now should be in 2PC)
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
      {can_you_commit, DestNodeName, _FromPid, RootNodeName,
       Trace} ->
	  % this is not for me -> find which node should take care of it
	  [{_DestNodeName, RouterPid}] = ets:lookup(Table,
						    DestNodeName),
	  % forward
	  New_Trace = [self()] ++ Trace,
	  RouterPid !
	    {can_you_commit, DestNodeName, self(), RootNodeName,
	     New_Trace},
	  io:format("~w ~w foward can you commit to ~w via "
		    "~w~n",
		    [RouterName, self(), DestNodeName, RouterPid]),
	  % only forward this cancommit request so Im not in 2PC
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
      {i_can_commit, RootNodeName, _FromPid, FromNodeName,
       Trace} ->
	  io:format("~w got i can commit of ~w ~n",
		    [RouterName, FromNodeName]),
	  % forward canCommit to rootnode
	  [LastPid | Rest] = Trace,
	  LastPid !
	    {i_can_commit, RootNodeName, self(), FromNodeName,
	     Rest},
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
      {i_cannot_commit, RootNodeName, _FromPid, FromNodeName,
       Trace} ->
	  [LastPid | Rest] = Trace,
	  LastPid !
	    {i_cannot_commit, RootNodeName, self(), FromNodeName,
	     Rest},
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
	  ...
	  io:format("routing table of ~w ~w : ~p~n",
		    [RouterName, self(), Obj]),
	  ...
	  ControlMessage = {control, self(), Pid, SeqNum,
			    ControlFun},
	  ...
	  propagate_control_message(ListofEntries, [],
				    ControlMessage),
	  Result = ask_nodes_for_commit(ListofEntries,
					RouterName),
	  io:format("result is ~w~n", [Result]),
	  if Result == timeout orelse
	       Result == abort orelse Children == abort ->
		 send_2nd_phase_msg(ListofEntries, SeqNum, RouterName,
				    doAbort),
		 if is_list(Children) == true ->
			lists:foreach(fun (NodePid) -> exit(NodePid, abort) end,
				      Children);
		 ...
		 send_2nd_phase_msg(ListofEntries, SeqNum, RouterName,
				    doCommit),
		 AllObj_from_tempTable = ets:match_object(TempTable,
							  {'$0', '$1'}),
		 ...
		 % finished 2PC
		 AllObj_from_table_now = ets:match_object(Table,
							  {'$0', '$1'}),
	  ...
	  if Cur_SeqNum < SeqNum ->
		 ControlMessage = {control, From, Pid, SeqNum,
				   ControlFun},
		 ListofEntries = ets:match_object(Table, {'$0', '$1'}),
		 propagate_control_message(ListofEntries, [],
					   ControlMessage),
      ...
      {doAbort, _SeqNum, DestNodeName, _FromPid, RootNodeName,
       Trace}
	  when DestNodeName == RouterName ->
	  ...
	  process(RouterName, Table, Cur_SeqNum, false, MsgQueue,
		  {});
      {doAbort, SeqNum, DestNodeName, _FromPid, RootNodeName,
       Trace} ->
	  % forwarding
	  New_Trace = [self()] ++ Trace,
	  [{_DestNodeName, RouterPid}] = ets:lookup(Table,
						    DestNodeName),
	  RouterPid !
	    {doAbort, SeqNum, DestNodeName, self(), RootNodeName,
	     New_Trace},
	  io:format("~w ~w foward doAbort to ~w via ~w~n",
		    [RouterName, self(), DestNodeName, RouterPid]),
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
      {doCommit, SeqNum, DestNodeName, FromPid, RootNodeName,
       Trace}
	  when DestNodeName == RouterName ->
	  io:format("~w ~w recv DoCommit from ~w~n",
		    [RouterName, self(), FromPid]),
	  ...
	  process(RouterName, Table, SeqNum, false, MsgQueue, {});
      {doCommit, SeqNum, DestNodeName, _FromPid, RootNodeName,
       Trace} ->
	  % forwarding
	  New_Trace = [self()] ++ Trace,
	  [{_DestNodeName, RouterPid}] = ets:lookup(Table,
						    DestNodeName),
	  RouterPid !
	    {doCommit, SeqNum, DestNodeName, self(), RootNodeName,
	     New_Trace},
	  io:format("~w ~w foward DoCommit to ~w via ~w~n",
		    [RouterName, self(), DestNodeName, RouterPid]),
	  process(RouterName, Table, Cur_SeqNum, IsIn2PC,
		  MsgQueue, TentativeResult);
	  ...
	  [LastPid | Rest] = Trace,
	  LastPid ! {doCommit, ack, RootNodeName, Rest},
	  io:format("~w ~w recv the doCommit ACK to ~w via "
		    "~w~n",
...
send_2nd_phase_msg([], _SeqNum, _RouterName,
		   _Command) ->
    ok;
send_2nd_phase_msg([FirstEntry | RestEntries], SeqNum,
		   RouterName, Command) ->
    {DestNodeName, RouteViaPid} = FirstEntry,
    if DestNodeName =/= '$NoInEdges' ->
	   io:format("send 2nd phase to ~w~n", [DestNodeName]),
	   RouteViaPid !
	     {Command, SeqNum, DestNodeName, self(), RouterName,
	      [self()]},
	   receive
	   ...
	   if NextDestNodeName == '$NoInEdges' ->
		  send_2nd_phase_msg(OtherEntries, SeqNum, RouterName,
				     Command);
	      true ->
		  send_2nd_phase_msg(RestEntries, SeqNum, RouterName,
				     Command)
	   end;
       true ->
	   send_2nd_phase_msg(RestEntries, SeqNum, RouterName,
...
ask_nodes_for_commit([], _RouterName) -> true;
ask_nodes_for_commit([FirstEntry | RestEntries],
		     RouterName) ->
    {DestNodeName, RouteViaPid} = FirstEntry,
    if DestNodeName =/= '$NoInEdges' ->
	   io:format("send cancommit to ~w~n", [DestNodeName]),
	   RouteViaPid !
	     {can_you_commit, DestNodeName, self(), RouterName,
	      [self()]},
	   receive
	     {i_can_commit, _Me, _FromPid, FromNodeName, _Trace}
		 when FromNodeName == DestNodeName ->
		 io:format("~w recv the canCommit from ~w~n",
			   [RouterName, FromNodeName]),
		 [NextEntry | OtherEntries] = RestEntries,
		 {NextDestNodeName, _NextRouteViaPid} = NextEntry,
		 if NextDestNodeName == '$NoInEdges' ->
	     ...
	     {i_cannot_commit, _Me, _FromPid, _FromNodeName,
	      _Trace} ->
		 abort
	     after 5000 -> timeout
	   end;
...
propagate_control_message([], Final_List,
			  ForwardMessage) ->
...
propagate_control_message([FirstEntry | RestEntries],
    ...
    if DestNodeName =/= '$NoInEdges' ->
	   ...
	   if Result == false -> New_List = [RouteViaPid] ++ List;
	      true -> New_List = List
	   end;
       true -> New_List = List
    end,
    propagate_control_message(RestEntries, New_List,
			      ForwardMessage).
