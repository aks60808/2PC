...
start(RouterName) ->
    ...
    SpawnPid = spawn(fun () ->
			     process(not_me, RouterName, Table, 0, false, [],
				     {})
		     end),
    % return Spawned Pid
    SpawnPid.
...
process(RootNode, RouterName, Table, Cur_SeqNum,
    ...
    receive
	  ...
	  process(RootNode, RouterName, Table, Cur_SeqNum,
		  IsIn2PC, New_MsgQueue, TentativeResult);
	  ...
	  if Cur_SeqNum == SeqNum andalso IsIn2PC == true ->
		 ...
		 process(RootNode, RouterName, Table, PreviousSeqNum,
			 false, MsgQueue, {});
	     true ->
	  ...
	  if Cur_SeqNum == SeqNum andalso IsIn2PC == true ->
		 ...
		 propagate_control_message(ListofEntries, [],
					   {doCommit, SeqNum, self(),
					    RootNodePid},
					   From, RootNodePid),
		 {_Children, TempTable, _PreviousSeqNum} =
		     TentativeResult,
		 ...
		 process(RootNode, RouterName, Table, SeqNum, false,
			 MsgQueue, {});
	     true ->
		 process(RootNode, RouterName, Table, Cur_SeqNum,
			 IsIn2PC, MsgQueue, TentativeResult)
	  end;
      {can_you_commit, DestNodeName, _DeliverPid,
       RootNodeName, Trace}
	  when DestNodeName == RouterName -> % I am the DestNode
	  if IsIn2PC == true ->
		 % check the tentative result
		 {Children, _TempTable, _PreviousSeqNum} =
		     TentativeResult,
	     ...
	     true -> ok % if current state is not in 2PC -> ignore it
	  end,
	  process(RootNode, RouterName, Table, Cur_SeqNum,
		  IsIn2PC, MsgQueue, TentativeResult);
      {can_you_commit, DestNodeName, _DeliverPid,
	  ...
	  process(RootNode, RouterName, Table, Cur_SeqNum,
		  IsIn2PC, MsgQueue, TentativeResult);
      {i_can_commit, RootNodeName, _DeliverPid, FromNodeName,
       Trace}
	  ...
	  process(RootNode, RouterName, Table, Cur_SeqNum,
		  IsIn2PC, MsgQueue, TentativeResult);
      {i_cannot_commit, RootNodeName, _DeliverPid,
       FromNodeName, Trace} ->
	  % send to root in opposite direction
	  [NextPid | Rest] = Trace,
	  NextPid !
	  ...
	  if Cur_SeqNum > SeqNum orelse Cur_SeqNum == SeqNum ->
		 Pid ! {abort, self(), SeqNum},
		 process(RootNode, RouterName, Table, Cur_SeqNum, false,
			 MsgQueue, TentativeResult);
	     true -> ok
	  end,
	  % 2PC phase 1
	  ...
	  if Result == timeout orelse
		 ...
		 % clean the mail box
		 flush(),
		 process(not_yet_set, RouterName, Table, Cur_SeqNum,
			 false, MsgQueue, {});
	     true ->
		 ...
		 flush(),
		 % finished 2PC
		 process(not_yet_set, RouterName, Table, SeqNum, false,
			 MsgQueue, {})
	  end;
      {control, From, Pid, SeqNum,
       ControlFun} -> % other nodes except root node
	  ...
	  if Cur_SeqNum < SeqNum andalso IsIn2PC == false ->
		 ...
		 process(RootNode, RouterName, Table, SeqNum, true,
			 MsgQueue, Updated_TentativeResult);
		 ...
		 {_PreChildren, Pre_TempTable, _Previous_SeqNum} =
		     TentativeResult,
...
ask_nodes_for_commit([FirstEntry | RestEntries],
    ...
    if DestNodeName =/= '$NoInEdges' ->
	   ...
	   receive
	     ...
	     {i_can_commit, _Me, _RouteFromPid, FromNodeName,
	      _Deliver_List}
...
propagate_control_message([], Final_List,
			  ForwardMessage, _FromPid, _RootNodePid) ->
    % sending control msg
...
propagate_control_message([FirstEntry | RestEntries],
			  List, ForwardMessage, FromPid, RootNodePid) ->
    {DestNodeName, RouteViaPid} = FirstEntry,
    if DestNodeName =/= '$NoInEdges' ->
	   ...
	   if Result == false andalso
		FromPid =/= RouteViaPid andalso
		  RootNodePid =/= RouteViaPid ->
    ...
    propagate_control_message(RestEntries, New_List,
			      ForwardMessage, FromPid, RootNodePid).
